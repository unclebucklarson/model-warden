...
| Husk removal + verified owner commands (2026-08-20) | Found in the field: `hf cache rm` exits 0 saying "Nothing to delete" for pruned husks — hf's cache scanner ignores snapshot-less repos, so the owner CLI *provably cannot* remove exactly the thing doctor asks it to. Doctor --fix looped forever claiming success. Two changes: owner-command success is now verified (`expect_gone` — the target must actually be gone; exit 0 is a claim, not proof), and husks became warden's second guarded direct-delete exception (same rationale as `*.incomplete`: no owner command can act) — re-verified at apply time to hold zero content bytes (refs-only, no symlinks, nothing over 4 KiB) with refusal deleting nothing. |
...
| Machine-level advisories (2026-08-20, M13.3) | Doctor's scope widens from store health to "is this machine protected": a `scrub timer off` advisory nags until the scrub runs, because install instructions get skipped but the tool people already run does not. Advisories follow the same remedy discipline (explanation, loss statement, executable fix on explicit action); non-systemd machines get silence, not noise. |
| Two-stage deletion (2026-08-25, M15, user decision) | The never-delete invariant gains its one sanctioned amendment: `warden delete` MOVES a bundle (same-fs rename — instant, free) into `<root>/.modelwarden/trash/`, restorable and invisible to scans; only `trash empty --yes` (CLI) / the Empty-Trash confirm (GUI) destroys bytes — two decisions separated in time. Companions a surviving model needs are auto-kept (bundle asymmetry). Foreign-store copies: owner commands are OFFERED (printed/copyable), never executed — weaker than doctor's --fix, per user decision. No auto-empty, ever — scheduled destruction sits badly with warden's character. |
| Test-first + M17 (2026-08-29, user decision) | Development is test-first from here (methodology in CLAUDE.md): failing test → implementation → refactor; decisions live in core for testability; bug fixes start as regression tests; tests/e2e.rs drives the real binary in isolated envs on every CI run. M17 (usability/perf, all TDD): backup-coverage headline + sortable column, GUI job queue, parallel (≤4) checkpointed hashing (resume-able first catalog), the operations journal (the durable activity stream persisted, plain-text cat-able), Settings dialog, and the stale-verification advisory (cold drives age out of trust at 90 days). |
